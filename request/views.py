    ...
    if request.method == "POST":
        ...
        if user != None:
            ...
            User.objects.filter ( username=Username ).update (
                pwd=base64.b64encode ( bytes ( Password, 'utf-8' ) ).decode () )
...
def env(request):
    ...
    try:
        env_list = Environment.objects.filter ( Q ( env_ip__contains=env_ip ), Q ( env_host__contains=env_host ),
                                                Q ( env_port__contains=env_port ) ).order_by ( "-id" )
    except:
...
def env_add_data(request):
    ...
    return render ( request, "./main/env.html",
                    {"envs": contacts, "isRepeat": isRepeat, "RepeatMessage": RepeatMessage} )
...
def env_search_name(request):
    ...
    contacts = paginator.page ( 1 )
    return render ( request, "./main/env.html",
                    {"envs": contacts, "env_ip": env_ip, "env_host": env_host, "env_port": env_port} )
...
def email(request):
    ...
    checkedenv_ids = request.GET.get ( "checkedenv_ids" )
    try:
        email_list = Email.objects.filter ( Q ( subject__contains=email_subject ),
                                            Q ( username__contains=email_username ),
...
def email_add_data(request):
    ...
    if isRepeat == 0:
        ...
        Email.objects.create ( sender=email_sender, receivers=email_receivers, host_dir=email_host_dir,
                               email_port=email_port, username=email_username, \
                               passwd=encryptionEmail_passwd, Headerfrom=email_Headerfrom, Headerto=email_Headerto,
    ...
    return render ( request, "./main/email.html",
                    {"emails": contacts, "isRepeat": isRepeat, "RepeatMessage": RepeatMessage} )
...
def database_add_data(request):
    ...
    return render ( request, "./main/database.html",
                    {"databases": contacts, "isRepeat": isRepeat, "RepeatMessage": RepeatMessage} )
...
def NosqlDatabase(request):
    ...
    checkedenv_ids = request.GET.get ( "checkedenv_ids" )
    try:
        NosqlDb_list = NosqlDb.objects.filter ( Q ( env_host__contains=host ),
                                                Q ( env_port__contains=port ) ).order_by ( "-id" )
    except:
...
def NosqlDatabase_add_data(request):
    ...
    return render ( request, "./main/NosqlDb.html",
                    {"NosqlDbs": contacts, "isRepeat": isRepeat, "RepeatMessage": RepeatMessage} )
...
def project(request):
    ...
    try:
        if select != '2':
            project_list = Project.objects.filter ( Q ( project_name__contains=project_name ),
...
def project_add_data(request):
    ...
    return render ( request, "./main/project.html",
                    {"projects": contacts, "select": '2', "isRepeat": isRepeat, "RepeatMessage": RepeatMessage} )
...
def project_search_name(request):
    ...
    if select != '2':
        project_list = Project.objects.filter ( Q ( project_name__contains=project_name ),
                                                Q ( Testers__contains=Testers ),
                                                Q ( Developer__contains=Developer ), Q ( status=select ) ).order_by (
            "-id" )
    else:
        project_list = Project.objects.filter ( Q ( project_name__contains=project_name ),
                                                Q ( Testers__contains=Testers ),
...
def modules_add_data(request):
    ...
    if isRepeat == 0:
        if status != None:
            Modules.objects.create ( Modules_name=Modules_name, Modules_desc=Modules_desc, Developer=developer,
                                     status=1, Project=project )
        else:
    ...
    return render ( request, "./main/modules.html",
                    {"modules": contactszip, "project_names": project_names, "isRepeat": isRepeat,
                     "RepeatMessage": RepeatMessage, "modulees": contacts} )
...
def modules_edit_data(request):
    ...
    isRepeat = len (
        Modules.objects.filter ( ~Q ( id=Modules_id ), Modules_name=Modules_name, Project=project ).values () )
    # 不重复则更新数据
    if isRepeat == 0:
        if status != None:
...
def modules_delete_data(request):
    ...
    project_names = get_project_name ( filiterdata )
    return render ( request, "./main/modules.html",
                    {"modules": contactszip, "project_names": project_names, "modulees": contacts} )
...
def modules_search_name(request):
    ...
    project_names = get_project_name ( filiterdata )
    return render ( request, "./main/modules.html",
                    {"modules": contactszip, "modules_name": modules_name, "selectproject": project_name, \
...
def get_modules_name(project_name, filiterdata):
    ...
    project = Project.objects.get ( project_name=project_name )
    modules_names = []
    startmodules_names = Modules.objects.filter ( Project=project, status=1, id__in=filiterdata_id_list ).values (
        "Modules_name" ).order_by ( "-id" )
        ...
        if selectproject == "0":
            if select != '2':
                case_list = Case.objects.filter ( Q ( case_name__contains=case_name ), Q ( api__contains=api ),
                                                  Q ( version__contains=version ), Q ( status=select ) ).order_by (
                    "-id" )
            else:
                case_list = Case.objects.filter ( Q ( case_name__contains=case_name ), Q ( api__contains=api ),
...
def case_add_data(request):
    ...
    if isRepeat == 0:
        if status != None:
            Case.objects.create ( case_name=case_name, case_desc=case_desc, api=api, version=version, status=1,
                                  Modules=modules )
        else:
            Case.objects.create ( case_name=case_name, case_desc=case_desc, api=api, version=version, status=0,
                                  Modules=modules )
        RepeatMessage = ""
    # 重复则不新增数据
    else:
    ...
    return render ( request, "./main/case.html",
                    {"cases": contactszip, "modules_names": modules_names, "project_names": project_names,
                     "casees": contacts, "isRepeat": isRepeat, "RepeatMessage": RepeatMessage} )
...
def case_edit_data(request):
    ...
    if isRepeat == 0:
        if status != None:
            Case.objects.filter ( id=case_id ).update ( case_name=case_name, case_desc=case_desc, api=api,
                                                        version=version,
...
def case_search_name(request):
    ...
    if selectproject == "0":
        ...
        if selectmodules == "0":
            ...
            if select != '2':
                ...
                case_list = Case.objects.filter ( Q ( case_name__contains=case_name ), Q ( api__contains=api ),
                                                  Q ( version__contains=version ), Q ( Modules=modules ) ).order_by (
                    "-id" )
    # 根据用户过滤数据权限
    filiterdata = get_filiterdata ( request )
    case_list = case_list.filter ( Modules__in=filiterdata )
    ...
    return render ( request, "./main/case.html",
                    {"cases": contactszip, "modules_names": modules_names, "project_names": project_names, \
                     ...
                     "selectmodules_names": selectmodules_names, "selectproject": selectproject,
                     "selectmodules": selectmodules, "casees": contacts} )
            ...
            if method == "0":
                if select == '2':
                    step_list = Step.objects.filter ( Q ( step_name__contains=step_name ),
                                                      Q ( steplevel__contains=steplevel ) ).order_by ( "-id" )
                else:
                    step_list = Step.objects.filter ( Q ( step_name__contains=step_name ),
                                                      Q ( steplevel__contains=steplevel ), \
                                                      Q ( status=select ) ).order_by ( "-id" )
            else:
                if select == '2':
                    step_list = Step.objects.filter ( Q ( step_name__contains=step_name ),
...
def get_weights(step_name, api_dependency):
    ...
    for variable in api_dependencyjson.keys ():
        for reference_step_name in api_dependencyjson[variable].keys ():
            oldreference_step_weights = \
            Step.objects.filter ( step_name=reference_step_name ).values ( 'step_weights' )[0]['step_weights']
...
def get_case_weights(step_name, api_dependency):
    ...
    for case_step_name in step_names:
        reference_step_names = Reference_step.objects.filter ( step_name=case_step_name['step_name'] ).values (
            'reference_step_name' )
        oldstep_weights = 0
        for reference_step_name in reference_step_names:
            reference_case_id = \
            Step.objects.filter ( step_name=reference_step_name['reference_step_name'] ).values ( 'case_id' )[0][
                'case_id']
            ...
            for reference_case_step_name in reference_case_step_names:
                old_case_step_weights = \
                    Step.objects.filter ( step_name=reference_case_step_name['step_name'] ).values ( 'step_weights' )[
                        0]['step_weights']
...
def step_add_data(request):
    ...
    case_names = get_case_name ( Case_dataqueryset )
    return render ( request, "./main/step.html",
                    {"steps": contacts, "case_names": case_names, "isRepeat": isRepeat, "RepeatMessage": RepeatMessage,
...
def step_edit_data(request):
    ...
    if isRepeat == 0:
        ...
        if method == "get" or method == "postform":
            if status != None:
                Step.objects.filter ( id=step_id ).update ( step_name=step_name, step_desc=step_desc,
                                                            steplevel=steplevel,
                                                            method=method, \
                                                            headers=headers, params=params, assert_response=asserts,
                                                            api_dependency=api_dependency, status=1, case=case )
    ...
    case_names = get_case_name ( Case_dataqueryset )
    return render ( request, "./main/step.html",
                    {"steps": contacts, "case_names": case_names, "step_names": step_names, "isRepeat": isRepeat,
...
def step_delete_data(request):
    ...
    case_names = get_case_name ( Case_dataqueryset )
    return render ( request, "./main/step.html",
                    {"steps": contacts, "case_names": case_names, "step_names": step_names} )
...
def step_search_name(request):
    ...
    if case_name == "0":
        ...
        if method == "0":
            ...
            if select == '2':
                ...
                step_list = Step.objects.filter ( Q ( step_name__contains=step_name ),
                                                  Q ( steplevel__contains=steplevel ), \
                                                  Q ( status=select ), Q ( method=method ), Q ( case=case ) ).order_by (
                    "-id" )
    # 根据登录用户获取相关数据
    Case_dataqueryset = get_filiterCasedata ( request )
    step_list = step_list.filter ( case__in=Case_dataqueryset )
    ...
    case_names = get_case_name ( Case_dataqueryset )
    return render ( request, "./main/step.html",
                    {"steps": contacts, "case_names": case_names, "select": select, "step_name": step_name, \
...
def get_step_name(Step_dataqueryset):
    step_names = []
    startstep_names = Step.objects.filter ( status=1, id__in=Step_dataqueryset ).values ( "step_name" ).order_by (
        "-id" )
...
def sql(request):
    ...
    try:
        if step_name == "0":
            if select != '2':
                if selectisselect != '2':
                    sql_list = Sql.objects.filter ( Q ( is_select=selectisselect ), Q ( status=select ) ).order_by (
                        "-id" )
                else:
...
def sql_add_data(request):
    ...
    if status != None:
        Sql.objects.create ( is_select=isselect, variable=variable, sql=sql, sql_condition=sql_condition, remake=remake,
                             status=1, step=step, db_remark=database_desc, db=db )
    else:
        Sql.objects.create ( is_select=isselect, variable=variable, sql=sql, sql_condition=sql_condition, remake=remake,
                             status=0, step=step, db_remark=database_desc, db=db )
    # 根据登录用户获取相关数据
    Step_dataqueryset = get_filiterStepdata ( request )
...
def sql_edit_data(request):
    ...
    if isselect == None:
        ...
        isselect = 1
    if status != None:
        Sql.objects.filter ( id=sql_id ).update ( is_select=isselect, variable=variable, sql=sql,
                                                  sql_condition=sql_condition, remake=remake, status=1, step=step,
                                                  db_remark=database_desc, db=db )
    else:
        Sql.objects.filter ( id=sql_id ).update ( is_select=isselect, variable=variable, sql=sql,
                                                  sql_condition=sql_condition, remake=remake, status=0, step=step,
                                                  db_remark=database_desc, db=db )
    # 根据登录用户获取相关数据
    Step_dataqueryset = get_filiterStepdata ( request )
    contacts = get_firstPagefiliterdata ( Sql, Step_dataqueryset )
...
def sql_search_name(request):
    ...
    return render ( request, "./main/sql.html",
                    {"sqls": contacts, "selectstep": step_name, "selectisselect": selectisselect, \
...
def Nosql(request):
    ...
    try:
        if step_name == "0":
            if select != '2':
                if selectisselect != '2':
                    Nosql_list = NoSql.objects.filter ( Q ( is_select=selectisselect ), Q ( status=select ) ).order_by (
                        "-id" )
                else:
            ...
            if select != '2':
                if selectisselect != '2':
                    Nosql_list = NoSql.objects.filter ( Q ( step=step ), Q ( is_select=selectisselect ),
                                                        Q ( status=select ) ).order_by ( "-id" )
                else:
                    Nosql_list = NoSql.objects.filter ( Q ( step=step ), Q ( status=select ) ).order_by ( "-id" )
            else:
                if selectisselect != '2':
                    Nosql_list = NoSql.objects.filter ( Q ( step=step ), Q ( is_select=selectisselect ) ).order_by (
                        "-id" )
                else:
...
def Nosql_add_data(request):
    ...
    if isselect == None:
        ...
        isselect = 1
    if status != None:
        NoSql.objects.create ( Nosql_dataType=Nosql_dataType, is_select=isselect, variable=variable, Nosql=Nosql,
                               Nosql_condition=Nosql_condition, remake=remake, status=1, step=step )
    else:
        NoSql.objects.create ( Nosql_dataType=Nosql_dataType, is_select=isselect, variable=variable, Nosql=Nosql,
                               Nosql_condition=Nosql_condition, remake=remake, status=0, step=step )
    # 根据登录用户获取相关数据
    Step_dataqueryset = get_filiterStepdata ( request )
...
def Nosql_edit_data(request):
    ...
    if isselect == None:
        ...
        isselect = 1
    if status != None:
        NoSql.objects.filter ( id=Nosql_id ).update ( Nosql_dataType=Nosql_dataType, is_select=isselect,
                                                      variable=variable, Nosql=Nosql, Nosql_condition=Nosql_condition,
                                                      remake=remake, status=1, step=step )
    else:
        NoSql.objects.filter ( id=Nosql_id ).update ( Nosql_dataType=Nosql_dataType, is_select=isselect,
                                                      variable=variable, Nosql=Nosql, Nosql_condition=Nosql_condition,
                                                      remake=remake, status=0, step=step )
    # 根据登录用户获取相关数据
    Step_dataqueryset = get_filiterStepdata ( request )
    contacts = get_firstPagefiliterdata ( NoSql, Step_dataqueryset )
    step_names = get_step_name ( Step_dataqueryset )
...
def Nosql_search_name(request):
    ...
    contacts = paginator.page ( 1 )
    step_names = get_step_name ( Step_dataqueryset )
    return render ( request, "./main/Nosql.html",
                    {"Nosqls": contacts, "selectstep": step_name, "selectisselect": selectisselect, \
...
def get_py_data(case_ids, testcasedir, task_name):
    ...
    for case_id in case_ids:
        ...
        # 得到外键数据
        case = Case.objects.get ( id=case_id )
        step_list_data = Step.objects.filter ( case=case, status=1 ).values ( "id", "step_name", "method", "params",
                                                                              "headers", "files", "assert_response",
                                                                              "api_dependency", "step_desc" )
        for step_data in step_list_data:
            # 得到外键数据
            step = Step.objects.get ( id=step_data['id'] )
            # sql数据
            sql_list_data = Sql.objects.filter ( step=step, status=1 ).values ( "sql_condition", "is_select",
                                                                                "variable", "sql", "db", "db_remark" )
            ...
            # Nosql数据
            nosql_list_data = NoSql.objects.filter ( step=step, status=1 ).values ( "Nosql_dataType", "Nosql_condition",
                                                                                    "is_select", "variable",
                                                                                    "Nosql" )
            step_data['sql_list_data'] = sql_list_data
...
def task(request):
    ...
    checkedenv_ids = request.GET.get ( "checkedenv_ids" )
    try:
        data_list = Task.objects.filter ( Q ( task_name__contains=task_name ) ).values ( "task_name",
                                                                                         "task_run_time_regular",
                                                                                         "db_remark", "env_desc",
                                                                                         "failcount", "remark",
                                                                                         "status", "subject",
                                                                                         "Nosqldb_desc" ).distinct ().order_by (
            'task_name' )
    except:
        data_list = Task.objects.values ( "task_name", "task_run_time_regular", "db_remark", "env_desc", "failcount",
                                          "remark", "status", "subject", "Nosqldb_desc" ).distinct ().order_by (
            'task_name' )
    paginator = Paginator ( data_list, NumberColumns )
    page = request.GET.get ( "page" )
    try:
...
def tasks_delete_data(request):
    ...
    return render ( request, "./main/task.html",
                    {"tasks": contacts, "env_descs": env_descs, "subjects": subjects, "Nosqldb_descs": Nosqldb_descs} )
...
def task_search_name(request):
    ...
    contacts = paginator.page ( 1 )
    return render ( request, "./main/task.html",
                    {"tasks": contacts, "task_name": task_name, "env_descs": env_descs, "subjects": subjects,
                     "Nosqldb_descs": Nosqldb_descs} )
    ...
    if status != None:
        if email_data == None:
            Task.objects.filter ( task_name=task_name ).update ( ip=env_id, Nosqldb=Nosqldb_id, failcount=failcount,
                                                                 task_run_time_regular=schedule, status=1,
                                                                 env_desc=env_desc, Nosqldb_desc=nosqldb_desc )
        else:
            Task.objects.filter ( task_name=task_name ).update ( ip=env_id, Nosqldb=Nosqldb_id, failcount=failcount,
                                                                 task_run_time_regular=schedule, status=1,
                                                                 env_desc=env_desc, Nosqldb_desc=nosqldb_desc,
        ...
        if email_data == None:
            Task.objects.filter ( task_name=task_name ).update ( ip=env_id, Nosqldb=Nosqldb_id, failcount=failcount,
                                                                 task_run_time_regular=schedule, status=0,
                                                                 env_desc=env_desc, Nosqldb_desc=nosqldb_desc )
        else:
            Task.objects.filter ( task_name=task_name ).update ( ip=env_id, Nosqldb=Nosqldb_id, failcount=failcount,
                                                                 task_run_time_regular=schedule, status=0,
...
def task_run(request):
    ...
    if schedule == None:
        # 拼接ip
        get_ip_database ( request, task_name, env_desc, nosdb_desc )
    ...
    try:
        data_list = Task.objects.filter ( Q ( task_name__contains=search_task_name ) ).values ( "task_name",
                                                                                                "task_run_time_regular",
                                                                                                "env_desc", "failcount",
                                                                                                "remark", "status",
                                                                                                "subject",
        ...
        data_list = Task.objects.values ( "task_name", "task_run_time_regular", "env_desc", "failcount", "remark",
                                          "status", "subject", "Nosqldb_desc" ).distinct ().order_by ( 'task_name' )
    paginator = Paginator ( data_list, NumberColumns )
    page = request.GET.get ( "page" )
    try:
        contacts = paginator.page ( page )
...
def task_list(request):
    ...
    tasklist['data'] = []
    taskdata = CarryTask.objects.filter ( task_name=task_name ).values ( 'id', 'stepcountnow', 'stepcountall',
...
def get_task_data(request):
    task_name_list = Task.objects.filter ( status=1 ).values ( "task_name", "task_run_time_regular", "env_desc",
                                                               "Nosqldb_desc", "failcount", "status", 'email',
                                                               'subject' ).distinct ().order_by ( 'task_name' )
    for i in range ( len ( task_name_list ) ):
        job = Job ( task_name_list[i]['task_name'], task_name_list[i]['task_run_time_regular'] )
        # 新建任务
        job.create_job ( request, task_name_list[i]['env_desc'], task_name_list[i]['Nosqldb_desc'],
                         task_name_list[i]['failcount'], task_name_list[i]['subject'] )
    ...
    if find_way == "findtextreport":
        ...
        if id == None:
            ...
            try:
                ...
                failmessage = "未生成断言错误日志"
        return render ( request, "../task/textreport.html",
                        {"successmessage": successmessage, "failmessage": failmessage} )
    # 查找html
...
def report(request):
    ...
    try:
        ...
        if len ( StatisticsData.objects.all () ) >= 1:
            StatisticsData.objects.update ( casenumber=casenumber, stepnumber=stepnumber, tasknumber=tasknumber,
                                            carrynumber=carrynumber,
                                            passnumber=passnumber, asserterrornumber=asserterrornumber,
                                            failnumber=failnumber,
                                            errorratio=errorratio * 100 )
        else:
            StatisticsData.objects.create ( casenumber=casenumber, stepnumber=stepnumber, tasknumber=tasknumber,
                                            carrynumber=carrynumber,
                                            passnumber=passnumber, asserterrornumber=asserterrornumber,
                                            failnumber=failnumber,
                                            errorratio=errorratio * 100 )
        ...
        for i in range ( test_carryTaskid, test_carryTaskid - 5, -1 ):
            passnumberlist.append (
                len ( LogAndHtmlfeedback.objects.filter ( test_carryTaskid=i, test_status=1 ).all () ) )
            failnumberlist.append (
                len ( LogAndHtmlfeedback.objects.filter ( test_carryTaskid=i, test_status=0 ).all () ) )
            asserterrornumberlist.append (
                len ( LogAndHtmlfeedback.objects.filter ( test_carryTaskid=i, test_status=2 ).all () ) )
        ...
        # 获取反馈信息
        feedbackmessages = LogAndHtmlfeedback.objects.values ( 'test_step', 'test_response', 'test_status',
        ...
        # 获取今日报错信息
        errorfeedbackmessages = LogAndHtmlfeedback.objects.filter ( ~Q ( test_status=1 ),
                                                                    update_time__gte=date ).values ( 'test_step',
                                                                                                     'test_response',
                                                                                                     'test_status',
